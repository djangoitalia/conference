...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cts = orm['contenttypes.ContentType'].objects.get(app_label='conference', model='speaker')
        for s in orm.Speaker.objects.all():
            if not s.user_id:
                # lascio fallire la migrazione, me ne preoccuperò quando ci
                # saranno speaker senza user
                1/0
            p = orm.AttendeeProfile()
            p.user = s.user
            p.slug = s.slug
            p.image = s.image
            p.personal_homepage = s.homepage
            p.twitter = s.twitter
            p.company = s.company
            p.company_homepage = s.company_homepage
            p.job_title = s.activity
            p.location = s.location
            p.save()
            for b in orm.MultilingualContent.objects.filter(content_type=cts, object_id=s.id):
                mc = orm.MultilingualContent()
                mc.content_type = ct
                mc.object_id = p.id
