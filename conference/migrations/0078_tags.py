...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for e in orm.Event.objects.all():
            tracks = dict([ (x.track, x) for x in orm.Track.objects.filter(schedule=e.schedule) ])
            tags = filter(lambda x: bool(x), e.track.split(' '))
            for ix, t in reversed(list(enumerate(tags))):
                if t in tracks:
                    del tags[ix]
                    track = tracks[t]
                    if orm.EventTrack.objects.filter(event=e, track=track).count() == 0:
                        orm.EventTrack(event=e, track=track).save()
            e.tags = ','.join(tags)
            e.save()
